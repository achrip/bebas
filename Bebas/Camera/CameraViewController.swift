...
class CameraViewController: UIViewController {
    ...
    lazy var model: ModelHand1? = {
        do {
            let config = MLModelConfiguration()
            return try ModelHand1(configuration: config)
        } catch {
    ...
    var onPredictPercentage: (([String : Double]) -> Void)?
...
extension CameraViewController {
    private func predictGesture(from points: [CGPoint]) {
        ...
        do {
            ...
            self.onPredictPercentage?(result?.wordProbability ?? ["null" : 0.0])
